...
const GAMES = {
  aarkTiles: aarkTilesGame,
};

// Get the current game (default to aarkTiles for backward compatibility)
function getCurrentGame() {
  ...
  const gameType = selectorGameType || urlGameType || "aarkTiles";
...
async function verify(event) {
  ...
  try {
    ...
    console.log("testestseset Computed hash:", gameData, gameState, computedHash);
    // Compare hashes
...
function initializeGameSelector() {
  ...
  console.log("Initializing game selector:", gameSelector);
  if (!gameSelector) return;

  // Set initial value based on URL param or default to aarkTiles
  const urlGameType = getParam("game");
  if (urlGameType && GAMES[urlGameType]) {
    gameSelector.value = urlGameType;
  } else {
    gameSelector.value = "aarkTiles";
  }
